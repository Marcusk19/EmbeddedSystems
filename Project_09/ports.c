...
//------------------------------------------------------------------------------
  ...
  P3SEL0 &= ~TEST_PROBE; // TEST_PROBE operation GPIO
  P3SEL1 &= ~TEST_PROBE; // TEST_PROBE operation GPIO
  ...
  P3SEL0 &= ~IR_LED;    // IR_LED operation set to GPIO
  P3SEL1 &= ~IR_LED;    // IR_LED operation set to GPIO
  P3DIR  |= IR_LED;     // set to output
  P3OUT |= IR_LED;      // output set high
  
  P3SEL0 |= IOT_LINK;   // IOT_LINK operation
  P3SEL1 |= IOT_LINK;   // IOT_LINK operation
  P3DIR &= ~IOT_LINK;   // set to input
  //P3OUT &= ~IOT_LINK;   // output set low
...
//------------------------------------------------------------------------------
  ...
  P5OUT &= ~IOT_RESET; // set output to low
  ...
  P5DIR &= ~IOT_PROG_SEL; // set to input
  ...
  P5DIR &= ~IOT_PROG_MODE; // set to input
