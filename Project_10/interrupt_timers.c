...
extern unsigned int ADC_enable_count;
unsigned int circle_clock = BEGINNING;
unsigned int black_line_count = BEGINNING;
unsigned int PID_count = BEGINNING;
...
__interrupt void TIMER0_B0_ISR(void){
  ...
  if(display_count > UPDATE_DISPLAY_THRESHOLD){         // 200 ms has passed
      ...
      if(clock_running) circle_clock+=TWO_HUNDRED_MS;
    }
