//==============================================================================
// Filename : switches.c
// Description: This file contains the functions for switches on the fram board
// 
// Author: Marcus Kok
// Date: January 2020
// Compiler: Made with IAR Embedded Workbench for MSP430 7.12.4
//==============================================================================

#include  "functions.h"
#include  "msp430.h"
#include "macros.h"
#include <string.h>

extern unsigned int buttonState;
extern char display_line[numLines][strSize];
extern unsigned int num_presses;
extern volatile unsigned char display_changed;
extern char display_line[numLines][strSize];
extern volatile unsigned char update_display;
extern unsigned int emitter_flag;
extern unsigned int black_line_flag;
extern int display_state;
extern volatile char process_buf[LARGE_RING_SIZE];
extern char receive_char[NCSU_SIZE];
extern int process_ready;
char serial_char_buffer[] = "  ";

void Switches_Process(void){
    if(emitter_flag){
      strcpy(display_line[line1], "  Emit.On ");
      update_string(display_line[line1], line1);
      enable_display_update();
      display_changed = TRUE;                        
    }
    else{
      strcpy(display_line[line1], "  Emit.Off");
      update_string(display_line[line1], line1);
      enable_display_update();
      display_changed = TRUE;
    }
    
    if(black_line_flag){
        strcpy(display_line[line1], " BLK LINE ");
        update_string(display_line[line1], line1);
        enable_display_update();
        display_changed = TRUE;
    }   
}

void Serial_Display_Process(void){
  if(display_state == TOGGLE_ONE){
    strcpy(display_line[line4], "  460800  ");
    update_string(display_line[line4], line4);
    enable_display_update();
    display_changed = TRUE;
  }
  
  if(display_state == TOGGLE_TWO){
    strcpy(display_line[line4], "  115200  ");
    update_string(display_line[line4], line4);
    enable_display_update();
    display_changed = TRUE;
  }
  if(process_ready){
    //strcat(receive_char, serial_char_buffer);
    strcpy(display_line[line1], receive_char);        // update display
    update_string(display_line[line1], line1);
    enable_display_update();
    display_changed = TRUE;
    process_ready = FALSE;
  }
}


